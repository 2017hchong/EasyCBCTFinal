...
namespace Lean.Touch
{
	...
	[HelpURL(LeanTouch.PlusHelpUrlPrefix + "LeanManualScaleX")]
	[AddComponentMenu(LeanTouch.ComponentPathPrefix + "Manual ScaleX")]
	public class LeanManualScaleX : MonoBehaviour
	{
		...
		[FSA("Space")] [SerializeField] private Space space;
		...
		[FSA("DirectionA")] [SerializeField] private Vector3 directionA = Vector3.right;
		...
		[FSA("DirectionB")] [SerializeField] private Vector3 directionB = Vector3.up;
		...
		[FSA("Multiplier")] [SerializeField] private float multiplier = 1.0f;
		...
		[FSA("Dampening")] [SerializeField] private float damping = 10.0f;
		...
		[FSA("ScaleByTime")] [SerializeField] private bool scaleByTime;
		...
		[FSA("DefaultPosition")] [SerializeField] private Vector3 defaultPosition;
		...
		public void TranslateAB(Vector2 magnitude)
		{
			...
			Translate(directionA * magnitude.x);
		}
		...
		private void UpdatePosition(float factor)
		{
			...
			finalTransform.localScale = Vector3.Scale(finalTransform.localScale, new Vector3(1, 1, 1) + (remainingDelta - newDelta));
			/*
			if (remainingDelta != new Vector3(0,0,0))
				Debug.Log(remainingDelta);
			*/
			remainingDelta = new Vector3(0, 0, 0);
		}
	}
}